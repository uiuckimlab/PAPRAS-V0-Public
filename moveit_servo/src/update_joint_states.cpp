...
void arm1_command_callback(const trajectory_msgs::JointTrajectory msg) {
    ...
    for(size_t i = 0; i < msg.joint_names.size(); i++){
        indexes[msg.joint_names[i]] = i;
    }
    
    for(size_t i = 0; i < publish_joint_state.name.size(); i++){
        if(indexes.find(publish_joint_state.name[i]) != indexes.end()){
            publish_joint_state.position[i] = msg.points[msg.points.size()-1].positions[indexes[publish_joint_state.name[i]]];
        }
        publish_joint_state.velocity[i] = 0;
    }
...
int main(int argc, char** argv)
{
    ...
    for(size_t i = 0; i < publish_joint_state.name.size(); i++){
        if(indexes.find(publish_joint_state.name[i]) != indexes.end()){
            publish_joint_state.position[i] = 0;
        }
        publish_joint_state.velocity[i] = 0;
    }
    new_joint_state_pub_.publish(publish_joint_state);
